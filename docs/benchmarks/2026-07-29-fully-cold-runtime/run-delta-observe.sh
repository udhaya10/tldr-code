...
before_file="$benchmark_dir/${label}-before.json"
started_file="$benchmark_dir/${label}-started-ms.txt"
poll_file="$benchmark_dir/${label}-poll.jsonl"

baseline_generation=$(jq -r '.artifact_store.active_generation' "$before_file")
started_ms=$(tr -d '[:space:]' <"$started_file")
: >"$poll_file"
...
for poll_index in $(seq 1 300); do
...
tldr daemon status -f json >"$benchmark_dir/${label}-after.json"
