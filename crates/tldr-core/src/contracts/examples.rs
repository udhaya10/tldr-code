...
fn generate_method_example(
    ...
    match kind {
        ApiKind::StaticMethod => {
            Some(format!("result = {class}.{method_name}({args})"))
        }
        ApiKind::ClassMethod => {
            Some(format!("result = {class}.{method_name}({args})"))
        }
        _ => {
            Some(format!("result = {var}.{method_name}({args})"))
        }
    }
}
...
pub fn example_for_type(type_ann: &str) -> String {
    ...
    if normalized.starts_with("Union[") {
        if let Some(inner) = normalized.strip_prefix("Union[").and_then(|s| s.strip_suffix(']')) {
