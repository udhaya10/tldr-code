...
#[cfg(test)]
pub(crate) fn write_legacy_generation(
    path: &Path,
    entries: impl IntoIterator<Item = (String, Vec<f32>, u64, Option<u64>)>,
) {
    let map = entries
        .into_iter()
        .map(|(key, embedding, cached_at, file_mtime)| {
            (
                key,
                LegacyCacheEntry {
                    embedding,
...
mod tests {
    ...
    #[test]
    fn migration_removes_legacy_only_after_every_sink_write_succeeds() {
        let directory = tempfile::tempdir().unwrap();
        let legacy = directory.path().join("cache.v1.00000000000000000001.rkyv");
        write_legacy_generation(
            &legacy,
            [
                ("a".into(), vec![1.0], 1, None),
                ("b".into(), vec![2.0], 2, Some(3)),
            ],
        );
        let mut calls = 0;
        assert!(migrate_latest(directory.path(), |_| {
            calls += 1;
            if calls == 2 {
                Err(TldrError::Embedding("injected migration failure".into()))
            } else {
                Ok(())
            }
        })
        ...
        let report = migrate_latest(directory.path(), |_| Ok(())).unwrap();
        assert_eq!(report.imported, 2);
        assert!(!legacy.exists());
    }

    #[test]
    fn corrupt_legacy_generation_has_clear_rebuild_error() {
        let directory = tempfile::tempdir().unwrap();
        std::fs::write(
            directory.path().join("cache.v1.00000000000000000001.rkyv"),
            b"corrupt",
        )
        .unwrap();
        let error = migrate_latest(directory.path(), |_| Ok(())).unwrap_err();
        assert!(error.to_string().contains("no valid legacy"));
    }
}
