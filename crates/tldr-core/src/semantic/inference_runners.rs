...
mod tests {
    ...
    #[test]
    fn runners_start_cold_and_distinct() {
        let query = FixedShapeInferenceRunner::query().snapshot();
        let delta = FixedShapeInferenceRunner::delta().snapshot();
        let bulk = BulkInferenceRunner::default().snapshot();
        assert_eq!(query.workload, InferenceWorkload::Query);
        assert_eq!(delta.workload, InferenceWorkload::Delta);
        assert_eq!(bulk.workload, InferenceWorkload::Bulk);
        assert!(query.state == "cold" && delta.state == "cold" && bulk.state == "cold");
    }

    #[test]
    fn bulk_state_is_observable_while_operation_runs() {
        let runner = Arc::new(BulkInferenceRunner::default());
        ...
        entered.wait();
        assert_eq!(runner.snapshot().state, "busy");
        release.wait();
        worker.join().unwrap();
