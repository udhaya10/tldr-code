...
mod tests {
    ...
    fn chunk(path: &Path, content: &str) -> CodeChunk {
        CodeChunk {
    ...
    #[test]
    fn per_record_put_get_and_reopen_roundtrip() {
        let directory = tempfile::tempdir().unwrap();
        let source = directory.path().join("a.rs");
        std::fs::write(&source, "fn a() {}").unwrap();
        let chunk = chunk(&source, "fn a() {}");
        let vector = vec![0.25; EmbeddingModel::ArcticXS.dimensions()];
        {
            let mut cache = EmbeddingCache::open(config(directory.path())).unwrap();
            cache.put(&chunk, vector.clone(), EmbeddingModel::ArcticXS);
            cache.flush().unwrap();
            assert_eq!(cache.len(), 1);
        }
        let mut reopened = EmbeddingCache::open(config(directory.path())).unwrap();
        assert_eq!(reopened.get(&chunk, EmbeddingModel::ArcticXS), Some(vector));
        assert_eq!(reopened.len(), 1);
    }

    #[test]
    fn exact_document_and_recipe_identity_control_hits() {
        let directory = tempfile::tempdir().unwrap();
        let source = directory.path().join("a.rs");
        std::fs::write(&source, "fn a() {}").unwrap();
        let chunk = chunk(&source, "fn a() {}");
        let mut cache = EmbeddingCache::open(config(directory.path())).unwrap();
        let recipe = EmbeddingRecipeId::for_document(EmbeddingModel::ArcticXS, "pipeline-one");
        let vector = vec![0.5; EmbeddingModel::ArcticXS.dimensions()];
        cache.put_document(&chunk, "composed", vector.clone(), &recipe);
        assert_eq!(
            cache.get_document(&chunk, "composed", &recipe),
            Some(vector)
        );
        assert!(cache.get_document(&chunk, "changed", &recipe).is_none());
        let stale = EmbeddingRecipeId::for_document(EmbeddingModel::ArcticXS, "pipeline-two");
        assert!(cache.get_document(&chunk, "composed", &stale).is_none());
    }

    #[test]
    fn redb_cache_is_explicitly_bounded_below_default_gibibyte() {
        let directory = tempfile::tempdir().unwrap();
        let cache = EmbeddingCache::open(config(directory.path())).unwrap();
        assert_eq!(cache.redb_cache_size_bytes(), 8 * 1024 * 1024);
        assert!(cache.redb_cache_size_bytes() < 1024 * 1024 * 1024);
    }

    #[test]
    fn invalid_dimensions_surface_at_flush_without_partial_record() {
        let directory = tempfile::tempdir().unwrap();
        let source = directory.path().join("a.rs");
        std::fs::write(&source, "fn a() {}").unwrap();
        let chunk = chunk(&source, "fn a() {}");
        let mut cache = EmbeddingCache::open(config(directory.path())).unwrap();
        cache.put(&chunk, vec![1.0], EmbeddingModel::ArcticXS);
        assert!(cache.flush().is_err());
        assert!(cache.is_empty());
    }

    #[test]
    fn legacy_rkyv_is_removed_only_after_successful_redb_import() {
        let directory = tempfile::tempdir().unwrap();
        let chunk = chunk(&directory.path().join("a.rs"), "fn a() {}");
        let key = CacheKey::from_chunk(&chunk, EmbeddingModel::ArcticXS, directory.path());
        let legacy_key = format!("{}:{}", encode_hex(&key.recipe), encode_hex(&key.revision));
        let legacy = directory.path().join("cache.v1.00000000000000000001.rkyv");
        crate::semantic::redb_migration::write_legacy_generation(
            &legacy,
            [(
                legacy_key,
                vec![0.75; EmbeddingModel::ArcticXS.dimensions()],
                unix_seconds(),
                None,
            )],
        );
        ...
        assert!(cache.get(&chunk, EmbeddingModel::ArcticXS).is_some());
    }

    #[test]
    fn corrupt_database_is_preserved_and_rebuilt() {
        let directory = tempfile::tempdir().unwrap();
        let database = directory.path().join(REDB_FILE);
        std::fs::write(&database, b"not-redb").unwrap();
        let cache = EmbeddingCache::open(config(directory.path())).unwrap();
    ...
    #[test]
    fn zero_day_ttl_is_checked_on_every_read() {
        let directory = tempfile::tempdir().unwrap();
        let source = directory.path().join("a.rs");
        std::fs::write(&source, "fn a() {}").unwrap();
        let chunk = chunk(&source, "fn a() {}");
        let mut cache = EmbeddingCache::open(CacheConfig {
            ttl_days: 0,
        ...
        cache.put(
            &chunk,
            vec![0.25; EmbeddingModel::ArcticXS.dimensions()],
            EmbeddingModel::ArcticXS,
        );
        assert!(cache.get(&chunk, EmbeddingModel::ArcticXS).is_none());
        assert!(cache.is_empty());
    }

    #[test]
    fn configured_payload_limit_evicts_oldest_records() {
        let directory = tempfile::tempdir().unwrap();
        let mut cache = EmbeddingCache::open(CacheConfig {
            max_size_mb: 1,
            ..config(directory.path())
        ...
        .unwrap();
        for index in 0..700 {
            let source = directory.path().join(format!("{index}.rs"));
            let chunk = chunk(&source, &format!("fn f{index}() {{}}"));
            cache.put(
                &chunk,
                vec![index as f32; EmbeddingModel::ArcticXS.dimensions()],
                EmbeddingModel::ArcticXS,
            );
        }
        cache.flush().unwrap();
        assert!(cache.stats().size_bytes <= 1024 * 1024);
        assert!(cache.len() < 700);
    }
}
