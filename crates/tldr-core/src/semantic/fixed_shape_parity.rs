...
mod tests {
    ...
    #[test]
    fn identical_finite_vectors_pass() {
        let parity =
            VectorParity::compare(&[0.6, 0.8], &[0.6, 0.8], ParityTolerance::default()).unwrap();
        assert!(parity.passed);
        assert_eq!(parity.max_absolute_difference, 0.0);
        assert!((parity.cosine_similarity - 1.0).abs() <= f32::EPSILON);
    }

    #[test]
    fn absolute_and_cosine_thresholds_fail_independently() {
        let absolute = VectorParity::compare(
            &[1.0, 0.0],
            &[0.999, 0.0],
            ParityTolerance {
                max_absolute_difference: 1e-5,
        ...
        assert!(absolute.cosine_similarity >= 0.99);

        let cosine = VectorParity::compare(
            &[1.0, 0.0],
            &[0.9, 0.4],
            ParityTolerance {
        ...
        .unwrap();
        assert!(!cosine.passed);
        assert!(cosine.max_absolute_difference <= 0.5);
    }

    #[test]
    fn malformed_vectors_fail_explicitly() {
        let tolerance = ParityTolerance::default();
        assert!(VectorParity::compare(&[1.0], &[1.0, 2.0], tolerance).is_err());
        assert!(VectorParity::compare(&[f32::NAN], &[1.0], tolerance).is_err());
        assert!(VectorParity::compare(&[0.0], &[0.0], tolerance).is_err());
    }

    #[test]
    fn empty_model_reports_never_pass_vacuously() {
        let report = ModelParityReport {
