...
//! mismatch): they spawn a real daemon, seed its cache with the default
//! (backward) query, then assert a forward / variable-filtered query still
//! gets the right answer. If anyone rewires slice/impact through a daemon
//! path that drops flags AND fixes the serialization shape, the seeded
//! backward cache entry makes these fail loudly.
//!
...
fn start_daemon(project: &Path) {
    ...
    // Let the detached child finish binding its socket before queries race it.
...
fn slice_json(file: &Path, extra: &[&str]) -> serde_json::Value {
...
fn slice_direction_survives_running_daemon() {
    ...
    let backward = slice_json(&file, &[]);
    let forward = slice_json(&file, &["-d", "forward"]);
    stop_daemon(project);
...
fn slice_variable_survives_running_daemon() {
    ...
    start_daemon(project);
    let _seed = slice_json(&file, &[]);
    let filtered = slice_json(&file, &["--variable", "x"]);
    stop_daemon(project);
